...
Cowork use cases mean using the agent to interact with knowledge work tools. That might include Jira, Confluence, Slack, Calendar, Email, GitHub, internal command-line clients, or anything else you use to do real work.

Codex permissions matter a lot when the project can touch work systems. At the same time, you do not want to be prompted to confirm every routine local operation. The goal is to make ordinary development and local context loading smooth, while preserving explicit confirmation before external side effects.

My permission goals are:
...
- Make permissions match the project's real workflow.
...
Worktrees are useful when using agents. They let you isolate a task without dirtying the main checkout. This matters when multiple AI sessions are working on related changes, and it also matters when you want to pause one experiment and start another.

Worktrees essentially create another working copy of your repo files. That is great for tracked files, but it creates a problem for local context. When a worktree is created, it does not copy files that are ignored by git, including your private local context files.
...
The migration is really about moving context into the repo in a way that multiple capable agents can use. Claude remains useful. Codex becomes the implementation loop. Shared skills prevent duplicated workflows. Permissions keep cowork automation safe. Worktrees keep agent work isolated.

The goal is not tool purity. The goal is a project that any capable agent can pick up and operate safely.
